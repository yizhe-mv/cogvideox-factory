...
def generate_video(model_path, prompt, lora_path, lora_name, output_file, fps):
    pipe = CogVideoXPipeline.from_pretrained(model_path, torch_dtype=torch.bfloat16).to(
        "cuda"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate video using CogVideoX and LoRA weights"
    )
    parser.add_argument(
        "--prompt", type=str, required=True, help="Text prompt for the video generation"
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        "--lora_name", type=str, default="lora_adapter", help="Name of the LoRA adapter"
    )
    parser.add_argument(
        "--output_file", type=str, default="output.mp4", help="Output video file name"
    )
    parser.add_argument(
